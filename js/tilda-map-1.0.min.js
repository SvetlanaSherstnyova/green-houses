function t_appendGoogleMap(recid, key) {
    if ("object" == typeof google && "object" == typeof google.maps) t_handleGoogleApiReady(recid); else if (window.googleapiiscalled) setTimeout((function () {
        t_appendGoogleMap(recid, key)
    }), 1e3); else {
        var runfunc = "window.t_handleGoogleApiReady_" + recid + ' = function () { t_handleGoogleApiReady("' + recid + '") }';
        eval(runfunc);
        var langPreferences = "", mapLang = "", tildaMapElement = document.querySelector("#rec" + recid + " .t-map"),
...
function t_handleGoogleApiReady(recid) {
    ...
    Array.prototype.forEach.call(gmaps, (function (gmap) {
        var arMarkers = window["arMapMarkers" + recid], mapstyle = "[]";
        ...
        var mapZoomAttr = gmap.getAttribute("data-map-zoom"),
            ...
            myOptions = {
                ...
                zoomControl: !0,
                styles: mapstyle
            }, map = new google.maps.Map(gmap, myOptions), bounds = new google.maps.LatLngBounds, marker;

        function t_map__attachInfoMessage(marker, descr) {
            var textarea = document.createElement("textarea");
            textarea.innerHTML = descr;
            var infowindow = new google.maps.InfoWindow({content: textarea.textContent});
...
function t_appendYandexMap(recid, key) {
    if ("object" == typeof ymaps && "function" == typeof ymaps.Map) t_handleYandexApiReady(recid); else if (window.yandexmapsapiiscalled) setTimeout((function () {
        t_appendYandexMap(recid, key)
    }), 1e3); else {
        var runfunc = "window.t_handleYandexApiReady_" + recid + ' = function () { return t_handleYandexApiReady("' + recid + '") }';
        eval(runfunc);
        var mapLang = "", tildaMapElement = document.querySelector("#rec" + recid + " .t-map"),
...
function t_handleYandexApiReady(recid) {
    ...
    Array.prototype.forEach.call(ymapsArr, (function (ymap) {
        ...
        var myLatLng = arMarkers.length > 0 && [parseFloat(arMarkers[0].lat), parseFloat(arMarkers[0].lng)],
            ...
            }, map = new ymaps.Map(ymap, myOptions), myGroup = new ymaps.GeoObjectCollection({}), eventsPane,
            eventsPaneEl = map.panes.get("events").getElement(), mobilePanelText = {
        ...
        arMarkers.length > 1 ? (map.setBounds(myGroup.getBounds(), {checkZoomRange: !0}).then((function () {
            ...
            map.container.fitToViewport(), arMarkers.length > 1 ? map.setBounds(myGroup.getBounds(), {checkZoomRange: !0}).then((function () {
                zoom > 0 && (0 == map.getZoom() || map.getZoom() > zoom) && map.setZoom(zoom)
            })) : zoom > 0 && (0 == map.getZoom() || map.getZoom() > zoom) && map.setZoom(zoom)
        }))
    }))
